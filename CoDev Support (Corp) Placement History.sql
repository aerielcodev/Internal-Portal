SELECT
    ...
    COALESCE(CONVERT(date,ce.DateEnd),CONVERT(date,emp.DeletedDate)) AS 'Placement End',
...
INNER JOIN (SELECT emp.*,e.Id, e.DeletedDate FROM Employees e INNER JOIN UserDetails emp ON emp.UserId = e.UserId) emp ON emp.Id = ce.EmployeeId
