SELECT 
    ...
    CONCAT(s.FirstName , ' ' , s.LastName) AS 'Talent Supervisor', 
    CONCAT(e2.FirstName ,' ' , e2.LastName) AS 'Team Member', 
...
JOIN [dbo].[FeedbackTypes] AS ft ON ft.[Id] = f.[FeedbackTypeId] 
...
WHERE f.FeedbackTypeId = 3 AND f.O3FeedbackDate IS NOT NULL AND f.IsDraft = 0
ORDER BY [O3 Date] DESC